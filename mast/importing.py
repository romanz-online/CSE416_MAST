    ...
    for row in profiles:
        ...
        if line != ['']:
            ...
            if line[4] and line[5] \
                ...
                student.track = Tracks_in_Major.objects.get(name=line[5],
    ...
    for row in course_plans:
        ...
        if line != ['']:
            ...
            if line[1] and Major.objects.filter(department=line[1]) and line[2]:
                ...
                if Course.objects.filter(department=department, number=int(line[2]), semester=semester,
                                         section=section):
                    new_class.course = \
                        Course.objects.filter(department=department, number=int(line[2]), semester=semester,
                                              section=section)[0]
                else:
