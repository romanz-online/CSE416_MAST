...
def get_unsatisfied_prerequisite(student, department, number):
    ...
    course_instance = CourseInstance.objects.filter(course=course_want_take).first()
    prerequisiteSet = CoursePrerequisiteSet.objects.filter(parent_course=course_instance)
    # add prerequisite to the list if the student has not take that course or failed
    if not prerequisiteSet:
        return []
    else:
        required_courses = []
